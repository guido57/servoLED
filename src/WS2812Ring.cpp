...
TaskWS2812Ring::TaskWS2812Ring(unsigned long interval, Scheduler *aS, void (*myCallback)(int * ndx)) : Task(interval, TASK_FOREVER, aS, true)
{
...
{
  // _myCallback();
  WS2812Ring_loop(&rainbowCycleNdx);
  return true;
};
...
void rainbowCycle(int * ndx) {
  ...
  //  Serial.printf("brightness=%d\n",br);
    //for(j=0; j<256*5; j++) { // 5 cycles of all colors on wheel
      ...
      for(i=0; i< strip->numPixels(); i++) {
        strip->setPixelColor(i, Wheel(((i * 256 / strip->numPixels()) + *ndx) & 255));
      }
      *ndx += 1;
